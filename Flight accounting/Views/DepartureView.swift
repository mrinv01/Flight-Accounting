...
struct DepartureView: View {
    ...
    var body: some View {
        VStack {
            ...
            List(flights.departures) { flight in
                if flights.departures.isEmpty {
                    Text("Нет рейсов")
                } else {
                    VStack(alignment: .leading) {
                        ...
                        HStack {
                            ...
                            Text("Отправление в \(formattedTime(flight.departureTime))")
                                .font(.title).fontWeight(.semibold)
                        }
                        HStack(alignment: .top) {
                            Spacer()
                            Text(" Прилетит: \(formattedDate(flight.arrivalDate)), \(formattedTime(flight.arrivalTime))")
                                .font(.subheadline)
                        }
                    }
                    .padding(.vertical, 10)
...
#Preview {
    ...
    return DepartureView(flights: testModel, isGuest: $isGuest)
}
